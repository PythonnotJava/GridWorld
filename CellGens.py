...
def generate_grid(width: int, height: int, num_obstacles: int, num_penalty: int, num_endpoints: int) -> tuple:
    ...
    while len(endpoints) < num_endpoints:
        ex, ey = random.randint(0, height - 1), random.randint(0, width - 1)
    ...
    while len(obstacles) < num_obstacles:
        ox, oy = random.randint(0, height - 1), random.randint(0, width - 1)
    ...
    def dfs(x, y, path):
        if (x, y) in endpoints:
            return True
        if not (0 <= x < height and 0 <= y < width) or grid[x][y] in {1, 2} or (x, y) in path:
            return False
...
if __name__ == "__main__":
    ...
    grid = generate_grid(width, height, num_obstacles, num_penalty, num_endpoints)
